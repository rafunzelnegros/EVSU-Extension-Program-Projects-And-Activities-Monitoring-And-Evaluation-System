...
class QuarterlyMonitoringReportForm(forms.ModelForm):
    class Meta:
        ...
        exclude = ['created_by', 'noted_extension_director_name']
        labels = {
        ...
        widgets = {
            ...
            'school_name': forms.TextInput(attrs={'readonly': 'readonly'}),
            'campus_name': forms.TextInput(attrs={'readonly': 'readonly'}),
            'date_conducted': DateInput(),
            'date_of_termination': DateInput(),
            'evaluation': forms.Textarea(attrs={'rows': 4, 'placeholder': 'Enter the monitoring and evaluation narrative...'}),
...
class UserManageForm(forms.Form):
    ...
    post_nominals = forms.CharField(max_length=120, required=False, label='Post-nominal letters', help_text='Optional, e.g. PhD, MPA, RN')
